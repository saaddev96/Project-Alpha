...
namespace UnityEngine.InputSystem.Samples.RebindUI
{
    ...
    public class RebindActionUI : MonoBehaviour
    {
            ...
            m_RebindOperation = action.PerformInteractiveRebinding(bindingIndex)
                ...
                .OnComplete(
                    operation =>
                    {
                        ...
                        if (DuplicateBinding(action, bindingIndex, allCompositeParts))
                        {
                            action.RemoveBindingOverride(bindingIndex);
                            CleanUp();
        ...
        private bool DuplicateBinding(InputAction action,int bindingIndex,bool allCompositeParts)
        {
            ...
            foreach (var binding in action.actionMap.bindings)
            {
                ...
                if(binding.effectivePath == inputBinding.effectivePath)
                {
                    ...
                    int charLenght = 1;
                    char indexChar = '/';
                    int subStringIndex = bindingPath.IndexOf(indexChar) + charLenght;
                    int lenght = bindingPath.Length - subStringIndex;
                    print($"{bindingPath.Substring(subStringIndex, lenght).ToUpper()} is already binded to {binding.action}");
                    m_WarningText.text = $"{bindingPath.Substring(subStringIndex, lenght).ToUpper()} is already binded to {binding.action}";
                    return true;
                }
            ...
            if (allCompositeParts)
            {
                for (int i = 1; i < bindingIndex; i++)
                {
                    ...
                    if (action.bindings[i].effectivePath == inputBinding.effectivePath)
                    {
                        ...
                        int subStringIndex = bindingPath.IndexOf(indexChar)+ charLenght;
                        int lenght = bindingPath.Length - subStringIndex;
                        print($"{bindingPath.Substring(subStringIndex, lenght).ToUpper()} is already binded to {action.bindings[i].action}");
                        m_WarningText.text = $"{bindingPath.Substring(subStringIndex, lenght).ToUpper()} is already binded to {action.bindings[i].action}";
                        return true;
                    }
                }
            }
        ...
        [Tooltip("text that will show Warning to player")]
